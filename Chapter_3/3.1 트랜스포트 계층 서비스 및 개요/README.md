...
- 트랜스포트 프로토콜은 종단 시스템에서 동장
   - 송신 측: 애플리케이션의 메시지를 세그먼트로 쪼개어 네트워크 계층으로 보냄
   - 수신 측: 세그먼트를 메시지로 재조립하여 애플리케이션 계층으로 보냄
- 애플리케이션이 사용가능한 트랜스포트 계층 프로코톨
   - 인터넷: TCP와 UDP
...
> ❇️ `네트워크 계층 프로토콜`은 **호스트들** 사이의 논리적 통신을 제공한다.

> ❇️ `트랜스포트 계층 프로토콜`은 각기 다른 호스트에서 동작하는 **프로세스들** 사이의 논리적 통신을 제공한다. 네트워크 계층의 서비스를 사용함.


<img width="247" height="191" alt="image" src="https://github.com/user-attachments/assets/c4e1bae3-fc4b-475d-a954-117bee752a3b" />
