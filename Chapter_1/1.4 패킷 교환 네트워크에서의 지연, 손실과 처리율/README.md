...
❇️패킷은 라우터 버퍼 안에서 대기 (이미지 추가 예정)
- 패킷 큐에서 자신의 차례를 기다림
- 링크의 패킷 도착 비율이 출력 링크의 용량을 초과할 경우 패킷 손실 가능 

<img width="1653" height="781" alt="KakaoTalk_20260922_173704169" src="https://github.com/user-attachments/assets/84502d0d-0c02-48a0-a137-95a114d04e1f" />
...
- `전파 지연(propagation delay)` <br/><br/>

❇️패킷지연 네 가지 종류 (이미지 추가 예정) <br/>
1. 노드 처리 지연 `dproc`
- 비트 오류 검사
- 출력 링크를 결정
- 일반적으로 < msec <br/><br/>
...
- 라우터의 혼잡 정도에 의존 <br/><br/>

3. 전송 지연 `dtrans`
- R = 링크 대역폭 (bps)
- L = 패킷 길이 (bits)
- 비트를 링크로 보내는 시간 = L/R <br/><br/>

4. 전파 지연 `dprop`
- d = 물리 링크의 길이
- s = 매체의 전송 속도 (~2x10^8 m/sec)
- 전파 지연 = d/s

🔶🔶 **지연 계산 문제 시험에 나옴**

<img width="1500" height="2000" alt="KakaoTalk_20260922_173937927_01" src="https://github.com/user-attachments/assets/9366d733-e7e2-4673-b5f7-64dd57f88f17" />
<img width="1500" height="2000" alt="KakaoTalk_20260922_173937927" src="https://github.com/user-attachments/assets/6d5a397d-a45e-4a83-b211-5b8b5ef11935" />
...
- a = 평균 패킷 도착 비율
    - 트래픽 강도 = La/R
        - La/R ~0: 평균 큐잉 지연이 작음
        - La/R = 1: 지연이 커짐
        - La/R > 1: 서비스 가능한 용량 이상의 패킷들이 도착. 평균 지연은 무한대
...
❇️패킷 손실
- 큐(버퍼)는 유한한 용량을 가짐
- 가득 차 있는 큐에 도착한 패킷은 드롭 됨(손실)

<br/>

❇️처리율: 출발지와 목적지 간에 전송되는 비트의 비율(비트/단위시간)
- 순간: 어느 한 시점에서의 비율
- 평균: 긴 시간에서의 비율
<img width="3000" height="4000" alt="KakaoTalk_20260922_174232023" src="https://github.com/user-attachments/assets/4c5c87d2-9a4c-4e58-8c1b-e6ec792c4063" />

<br/><br/> 

❇️처리율: 인터넷 시나리오
- 연결 당 종단 간 처리율: (Rs, R/10, Rc) 중 최소값
- 실제 상황에서는 접속 네트워크에 해당하는 Rc 또는 Rs에서 빈번한 병목현상 발생
...
✔️ `La/R > 1`이라면 **비트가 큐에 도착하는 평균율이 비트가 큐에서 전송되는 비율을 초과한다**는 것을 의미한다.

따라서 큐는 끝없이 증가, 큐잉 지연은 무한대에 도달한다. *→ 트래픽 강도가 1보다 크지 않게 시스템을 설계해야 한다.*

<br/>

✔️ `La/R ≤ 1`인 경우에는 도착 트래픽의 특성이 큐잉 지연에 영향을 미친다.
...
❇️아래 그래프는 **트래픽 강도에 대한 평균 큐잉 지연의 질적 의존도**를 나타낸다.
