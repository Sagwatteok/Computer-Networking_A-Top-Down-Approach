...
❇️호스트(종단 시스템)는 인터넷의 가장자리에 연결되어 애플리케이션을 실행하는 컴퓨터 및 장치들로, 클라이언트와 서버로 분류된다.
...
❇️라우터에 물리적으로 연결해 주는 통로이다. (라우터: 서로 다른 네트워크 간에 데이터를 중계해주는 장비)
...
❇️가정용 인터넷은 DSL(전화선), 케이블(TV선), FTTH(광케이블) 세 가지 방식으로 제공되며, 기존 인프라를 재활용하거나 새로운 기술을 사용한다.
...
❇️DSL은 이미 깔려있는 기존 전화선을 재활용해서 전화와 인터넷을 동시에 쓸 수 있게 해주는 기술이며, 새로 케이블을 깔지 않아도 돼서 저렴하게 인터넷을 제공할 수 있기 때문에 쓴다.
...
❇️DSL 모뎀은 디지털 데이터를 고주파 신호로 변환해 전화선으로 보내고, 중앙국의 DSLAM이 이를 다시 디지털로 변환하여 인터넷과 연결한다.
...
❇️하나의 전화선을 서로 다른 주파수 대역으로 나누어 전화 신호와 인터넷 데이터를 동시에 전송한다.
...
❇️가정에서 도착한 신호를 전화와 인터넷 데이터로 분리해서 각각 전화기와 DSL 모뎀으로 보낸다.
...
❇️중앙국에서 여러 가정의 신호를 받아 데이터와 전화를 분리한 후, 데이터를 인터넷으로 전송한다.
...
❇️다운스트림이 업스트림보다 빠른 비대칭 전송률을 정의하며, 최신 표준은 1 Gbps 속도를 지원한다.
...
❇️케이블 TV 회사의 기존 인프라를 재활용해서 인터넷을 제공하며, 광케이블과 동축케이블을 함께 사용한다.
...
❇️케이블 TV 신호를 가공, 증폭, 분배하는 중계국으로, 인터넷 데이터도 여기서 처리된다.
...
❇️케이블 인터넷 접속을 위한 모뎀으로, 이더넷 포트로 PC에 연결되고 CMTS와 통신한다.
...
❇️헤드엔드에 위치하며 케이블 모뎀으로부터 받은 아날로그 신호를 디지털로 변환하는 장치로, DSL의 DSLAM과 같은 역할을 한다.
...
❇️모든 패킷이 다운스트림 채널로 모든 가정에 전달되고 업스트림도 공유되기 때문에, 사용자가 많으면 실제 수신율이 다운스트림 전송률보다 떨어진다.
...
❇️광케이블을 가정까지 직접 깔아서 가장 빠른 인터넷을 제공하는 방식이다.
...
❇️다이렉트 광섬유는 중앙국에서 각 가정까지 별도의 광섬유를 직접 연결하는 FTTH의 가장 단순한 방식
...
❇️광섬유를 여러 가정이 공유하기 위해 하나의 광섬유를 개별 광섬유로 분리하는 두 가지 구조로, AON은 교환형, PON은 수동형이다.
...
- `PON` : 아래 그림 참고 (PON 분배 구조를 이용하는 FTTH 인터넷 접속)
  ...
  이처럼 각 가정에서의 사용자는 홈 라우터를 ONT에 연결하고, 그를 통해 인터넷에 접속한다.<br/>
  ※ OLT: 각 가정에 설치되며 광신호를 받아서 홈 라우터와 연결하는 장치 <br/><br/>

  인터넷 접속 절차는 아래와 같이 정리할 수 있다. <br/>
...
❇️빔포밍 기술로 기지국에서 가정의 모뎀으로 무선 데이터를 전송하는 방식으로, 케이블이나 광케이블을 깔지 않아도 된다. <br/>

- `빔포밍(beam-forming) 기술`을 이용하여 서비스 제공가의 기지국에서 가정 내의 모뎀으로 데이터를 **무선으로** 전송한다.
    - 빔포닝: 무선 신호를 특정 방향으로 집중시켜 보내는 기술
...
❇️종단 시스템을 가장자리 라우터에 연결하는 로컬 네트워크로, 이더넷이 기업, 대학, 가정에서 가장 널리 사용된다.
...
❇️꼬임쌍선으로 이더넷 스위치에 연결되며, 스위치들이 네트워크를 형성해서 더 큰 인터넷으로 연결되는 유선 LAN 기술이다.
...
❇️AP(Access Point)를 통해 스마트폰, 태블릿 같은 기기들이 무선으로 인터넷에 접속하며, WiFi(IEEE 802.11)가 가장 일반적인 표준이다.
...
❇️광역 무선 접속은 이동 전화망 사업자의 기지국을 이용해 와이파이보다 훨씬 넓은 수십 킬로미터 범위에서 무선으로 인터넷에 접속할 수 있는 기술이다.
...
> ❇️비트는 **물리 매체(physical media)상에** 전자파나 광 펄스를 전파하여 전송한다.
...
❇️가장 저렴하고 널리 사용되는 전송 매체로, 두 개의 절연 구리선이 나선형으로 꼬여 있으며 LAN에서 주로 사용되고 Category 급수가 높을수록 빠르다.

- 가장 싸고 가장 많이 이용하는 전송 매체 (전화기에서 전화국 스위치까지 유선 연결의 99% 이상이 이를 이용)
- 구성
    - **두 개의 절연 구리선**, 각각은 약 1mm 굵기로 **규칙적인 나선 형태**로 배열된다.
...
- 사용 : UTP(Unshielded Twisted Pair) - 빌딩의 컴퓨터 네트워크, LAN에서 가장 많이 이용하는 매체
    - Category 급수에 따라 전송 속도 다름 -> 급수가 클수록 빠름

<br/>

### 동축케이블

❇️꼬임쌍선보다 더 높은 전송률을 제공하며, 동심원 구조로 되어 있고 케이블 TV 시스템에 주로 사용되며 여러 기기가 공유할 수 있다.
...
- Baseband : 케이블에 단일 채널 (예전의 이더넷)
- Broadband : 케이블에 다중 채널

<br/>

### 광섬유

❇️빛 펄스로 초당 10~100기가비트의 매우 높은 속도로 데이터를 전송하며, 오류율이 낮고 간섭에 강하지만 고가라서 장거리 전송에만 주로 사용된다.

- 광 펄스를 전달하는 광섬유(glass fiber)
    - 비트를 나타내는 빛의 파동을 전하는 가늘고 유연한 매체
- 초당 10~100기가비트에 이르는 높은 비트율을 지원한다. (고속동작)
...
❇️물리적 선 없이 전자기 스펙트럼을 통해 신호를 전송하는 무선 매체로, 반사나 간섭 같은 환경 요인의 영향을 받으며 지상파, LAN, 광역, 위성 등 다양한 무선 링크에 사용된다.

- 신호가 전자기 스펙트럼을 통해 전달
- 물리적 "선"이 없음
- 양방향
- 전파에 영향을 미치는 환경 요인: 반사, 물체에 의한 장애, 상호 간섭
- 무선 링크 종류
    - 지상 마이크로파
    - Lan (ex. WIFI)
    - 광역 (ex. 셀룰러)
