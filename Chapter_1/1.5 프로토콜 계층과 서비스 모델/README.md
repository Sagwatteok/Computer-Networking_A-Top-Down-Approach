...
- 네트워크는 복잡함
- 많은 구성 요소들
    - 호스트, 라우터, 다양한 링크 수준의 매체, 응용, 프로토콜, 하드웨어/소프트웨어
...
- 명시적인 구조가 복잡한 시스템 구성 요소를 식별하고 관계를 잘 알 수 있게 해줌
    - 논의를 위해 계층화된 참조 모델이 존재
- 모듈화는 시스템의 갱신, 유지보수를 쉽게 해줌
    - 계층에서 제공하는 서비스 구현의 변경은 나머지 시스템에 투영
    - ex. 게이트에서의 절차를 바꾸는 것이 나머지 시스템에 영향을 미치지 않음
- 계층화를 고려하는 게 위험한가?

<br/>

❇️인터넷 프로토콜 스택
- 애플리케이션: 네트워크 애플리케이션을 지원
    - FTP, SMTP, HTTP
- 트랜스포트: 프로세스 간 데이터 전송
    - TCP, UDP
- 네트워크: 출발지에서 목적지까지 데이터그램을 라우팅
    - IP, 라우팅 프로토콜
- 링크: 이웃한 네트워크 구성 요소들 간의 데이터 전송
    - PPP, 이더넷
- 물리: 전송 매체 위의 비트
...
❇️ISO/OSI 참조모델
- 프레젠테이션: 애플리케이션으로 하여금 데이터의 의미를 해석할 수 있도록 해줌
    - ex. 암호화, 압축, 특정 기계에 특화된 행동
- 세션: 동기화, 체크 포인트 설정, 데이터 교환의 경계
- 인터넷 프로토콜 스택에서는 프레젠테이션 및 세션 계층이 없음
    - 이들 서비스는 필요할 경우 애플리케이션에서 구현되어야 함
