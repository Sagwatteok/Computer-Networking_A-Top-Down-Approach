...
## ❇️공격자는 인터넷을 통해 여러분의 호스트에 멀웨어(악성코드)를 침투시킬 수 있음

#### 바이러스
- 기존 파일이나 프로그램에 추가되어 대상을 오염시키고 사용자가 오염된 프로그램을 실행
하는 능동적 개입(예: e-mail 첨부 파일 열기, 감염된 프로그램 실행하기)을 통해 감염이
확산되는 형태
- 자기복제: 자신을 다른 호스트, 사용자로 전파

#### 웜
- 사용자의 개입없이 독립적으로 작동하며 시스템의 취약점을 이용하여 네트워크를 통해 감
염이 전파되는 형태
- 자기복제: 자신을 다른 호스트, 사용자로 전파

<br/>

스파이웨어는 키보드 입력, 웹 사이트 방문기록 등을 기록해 특정한 사이트로 보냄. <br/>
감염된 호스트는 봇넷에 등록되어 스팸과 분산서비스 공격에 사용됨.
...
> 공격자들이 불필요한 트래픽을 보내어 정상적인 사용자들이 자원(서버, 대역폭)을 사용할 수 없게 하는 것
...
> 응용 또는 운영체제의 취약한 부분을 공략하는 메시지를 전송하여 서비스 중단을 유도
...
❇️DoS 공격 절차의 예시
1. 목표를 선택
2. 목표 주변 네트워크에 있는 호스트에 침입
3. 침입한 호스트에서 목표로 패킷을 보냄
...
❇️패킷 Sniffing
- 방송되는 매체(공유된 이더넷 무선)
- Promiscuous 인터페이스 카드는 지나가는 모든 패킷을 읽고 기록 (비밀번호도 포함해서)
- Wireshark 프로그램은 무료로 제공되는 패킷-Sniffer
