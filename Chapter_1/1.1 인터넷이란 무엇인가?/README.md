...
❇️이더넷은 한정된 지역 내 기기 연결 기술(LAN)이고, 인터넷은 전 세계 네트워크들을 연결한 네트워크의 네트워크이다
- 학교 학생식당 테이블에서 무선으로 와이파이에 연결되는 상황
- 학교 기숙사에서 서울의 친구에게 카톡을 보내는 상황
...
❇️호스트/종단 시스템은 인터넷에 연결된 모든 기기들을 뜻하며, 실제 데이터를 주고받는 출발점이자 목적지이다.
...
❇️통신 링크는 호스트(기기)들 사이에 패킷(데이터)을 전송하는 물리적 연결 통로이며, 얼마나 빨리 전송할 수 있는지를 전송률로 표현한다.
...
❇️패킷은 원본 데이터(큰 데이터)를 작은 조각(세그먼트)으로 나누고 헤더를 붙인 것으로, 목적지에서 다시 조립된다.
...
❇️패킷 스위치는 도착한 패킷을 올바른 출력 링크를 통해 목적지 방향으로 전달하는 장비이다.
...
❇️경로는 패킷이 출발지에서 목적지까지 거쳐가는 모든 통신 링크와 스위치들의 순서이다.
...
❇️ISP는 종단 시스템(개인, 기업)과 인터넷을 연결해주는 통신사로, 여러 ISP들이 계층 구조로 서로 연결되어 인터넷을 형성한다.
- ex. ISP 덕분에 유튜브 같은 회사들이 인터넷에 직접 연결되고, 사람들은 그 서버에서 영상을 볼 수 있음

<br/>

- 패킷 스위치와 통신 링크로 이루어진 네트워크
- 종단 시스템에게 다양한 네트워크 접속을 제공한다. (가정용 초고속 접속, 고속 LAN 접속, 이동 무선 접속 등)
- CP(content provider 컨텐츠 제공자)에게 인터넷 접속을 제공 → 웹 사이트나 비디오 서버를 인터넷에 직접 연결할 수 있게 된다.
- ISP들의 상호 연결
...
❇️프로토콜은 네트워크에서 데이터를 주고받을 때 지켜야 할 통신 규칙이며, TCP/IP는 인터넷 통신의 핵심 규칙이다.
...
❇️인터넷은 여러 종단 시스템에서 실행되는 애플리케이션들이 데이터를 주고받을 수 있게 해주는 기반 인프라이며, 라우터는 단지 데이터 전달만 담당한다.
...
❇️소켓 인터페이스는 애플리케이션이 인터넷을 통해 다른 기기의 프로그램으로 데이터를 보낼 때 따라야 하는 규칙의 집합이다.
...
❇️프로토콜은 둘 이상의 통신 개체가 데이터를 주고받을 때 함께 따라야 하는 규칙으로, 같은 프로토콜을 쓰지 않으면 상호작용이 불가능하다.
...
❇️인터넷의 모든 통신 활동(송수신, 경로 설정, 혼잡 제어 등)은 프로토콜이 정의한 메시지 포맷, 순서, 행동 규칙에 따라 이루어진다.
