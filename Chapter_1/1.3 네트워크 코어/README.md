...
1. `패킷 교환(packet switching)` : 자원이 보장되지 않음 → 필요할 때마다 경쟁해서 사용 (e.g., 인터넷)
2. `회선 교환(circuit switching)` : 자원을 미리 예약 → 보장된 대역폭 확보 <br/><br/>

- 인터넷: 서로 연결된 라우터의 그물망
- 근본적인 질문: 데이터가 어떻게 네트워크를 통해 전달되나?
    - 패킷 교환: 데이터가 "조각" 분리되어 네트워크를 통해 전달
        - 패킷들을 라우터에서 다음 라우터로 전달
        - 패킷은 출발지에서 목적지까지의 경로를 따라 전달됨
        - 각 패킷은 라우터와 라우터 사이의 링크의 전체 전송 용량을 사용해 전달함
    - 회선 교환: 호출(call)마다 전용 회선 사용 -> 전화 네트워크
...
3. 패킷은 **링크의 최대 전송률과 같은 속도로** 각각의 통신 링크에서 전송된다.
    - 공식: 전송 시간 = **L/R**
        - L = 패킷 크기(비트)
        - R = 링크 속도(bps)
        - 예) 7.5Mbit를 1.5Mbps 링크로 전송 = 7.5/1.5 = 5초
...
- 전송 지연: R bps의 링크 상에 L비트의 패킷을 전송하는데 L/R초가 소요됨
- 저장 후 전송: 패킷 전체가 라우터에 도달한 이후에야 다음 링크로 다시 전송될 수 있음
- End-end 지연: 2L/R (전파 지연은 없다고 가정)
    - L = 7.5 Mbits
    - R = 1.5 Mbps
    - 1Hop 전송지연 = 5초
    - 전체 전송지연 = 10초
...
만약 짧은 기간 동안 라우터에 도착하는 패킷의 전송률이 15 Mbps를 초과하게 된다면, 링크의 출력 버퍼에 패킷들이 큐잉될 것이다. <br/><br/>

❇️패킷 큐잉 및 손실
- 정해진 시간 동안에 링크로의 패킷 도달율이 링크를 통한 패킷 전송률보다 클 경우:
    - 패킷은 링크에서 전송을 기다리며 **큐에서 대기**함
    - 만약 대기 큐의 버퍼가 가득 차는 경우에는 도달한 **패킷이 손실**될 수 있음
...
- 라우팅(routing): 패킷이 전달될 출발지-도착지 경로 결정
...
❇️회선 교환 방식에서는 "call"에 대해 종단 간 회선 자원을 예약하여 사용
- 예시에서 각 링크는 4개의 회선으로 구성됨 (사진 추가 예정)
    - 상위 링크에서는 두 번째 회선을 예약
    - 우측 링크에서는 첫 번째 회선을 예약
- 전용 자원: 공유하지 않음.
    - 예약된 회선을 통해 보장된 성능 보장
- Call에 의해 예약된 회선이 사용되지 않으면 회선 자원이 낭비됨 (회선이 다른 사용자와 공유되지 않기 때문)
- 회선 교환은 주로 전통적인 전화 네트워크에서 사용됨
- 회선 사용 전에 호 설정(call setup)이 필요한 특성이 있음
...
2. `시분할 다중화(Time-Division Multiplexing, TDM)` : 각 회선은 <b>주기적으로</b>(짧은 시간 즉, 슬롯 동안) **전체** 대역폭을 얻는다. <br/><br/>

❇️FDM: 주파수 분할 다중화
- 주파수 대역을 더 작은 여러 개의 대역으로 나눔
- 나누어진 작은 주파수 대역을 사용자에게 할당하고 해당 주파수 대역을 사용하도록 함 <br/>

❇️TDM: 시간 분할 다중화
- 시간을 더 작은 크기의 슬롯들로 나눔
- 각 사용자에게 슬롯을 할당하고 해당 슬롯 구간에서 데이터를 전송함
...
❇️패킷 교환이 더 많은 사용자들이 네트워크를 사용할 수 있게 해줌
- 1Gb/s 링크
- 각 사용자
    - 사용 시 100Mb/s 대역폭 필요
    - 전체 시간 중 10% 사용
- 회선 교환
    - 10명의 사용자가 동시 사용 가능
- 패킷 교환
    - 35명의 사용자일 경우, 10명이 동시에 사용할 확률이 0.0004보다 낮음 <br/><br/>

❇️패킷교환이 진정한 승자인가?
- 패킷교환은 bursty 데이터 전송에 유리
    - 자원 공유
    - 단순함, 호 설정이 필요 없음
- 과도한 혼잡 -> 패킷 지연과 손실 발생 가능
    - 안정적인 데이터 전송과 혼잡 제어를 위한 프로토콜 필요
- 질문: 어떻게 회선 교환과 비슷한 동작을 제공하나?
    - 오디오/비디오 응용에서 필요로 하는 대역폭 보장
    - 해결해야 할 문제들이 있음 (7장)
...
✔️ `회선 교환`의 경우, 100 kbps가 항상 각각의 사용자에게 예약되어야 한다.  
...
✔️ `패킷 교환`의 경우, 한 특정 사용자가 활동을 하고 있을 확률은 10%이다.  
...
- 단말 시스템은 접속(access) ISP(Internet Service Provider)들을 통해 인터넷에 연결됨
- 접속 ISP들은 순차적으로 연결되어야함
    - 임의의 두 단말은 상호 간 패킷 전송 가능
- 결과적으로 네트워크들의 네트워크는 상당히 복잡함
- 현재의 인터넷 구조를 점진적으로 살펴봄 <br/><br/>
...
- Tier 1 ISP: 국가 간 연결
- Tier 2 ISP: 지역/국가 단위 ISP, Tier 1에 요금 지불하고 연결(고객 관계) + Tier 2끼리 피어링도 가능
- Tier 3 ISP(access/local ISP): 가정·기업 등 최종 사용자와 직접 연결되는 최하위 ISP, Tier 2에 연결
- 콘텐츠 공급자 네트워크 (구글, MS 등): 기존 ISP를 사용하지 않고 자신만의 네트워크를 운영하여 하위 ISP 연결을 통해 사용자들에게 접근 가능 -> 사설 네트워크의 데이터 센터를 지역 ISP들과 연결
